...
def plot_histograms(
    ...
    for i, col in enumerate(columns):
        sns.histplot(
            ...
            kde=True,
            ax=axs[i],
            color=sns.color_palette(palette)[i % len(columns)],
        )
...
def plot_ranked_boxplot(
    ...
    matplotlib.figure.Figure
    """
    df_ranked = pd.DataFrame({"x_ranked": x_ranked, "y_ranked": y_ranked})
    fig, ax = plt.subplots(figsize=figsize)
...
def plot_ecdf_comparison_multi(
    ...
    for (label, (x, y)), color, ls in zip(
        ecdf_data.items(), palette, linestyles * (n_curves // 4 + 1)
    ):
...
def plot_sampling_distributions(
    ...
    for (label, sample), color in zip(samples_dict.items(), colors):
        ...
        sns.histplot(sample, bins=bins, stat="density", label=label, alpha=0.5, color=color, ax=ax)
...
def plot_bootstrap_distribution(
    ...
    ax.axvline(
        mean_val,
        color=sns.color_palette("Blues", 8)[6],
        linestyle="-",
        linewidth=2,
    ...
    if ci_bounds:
        ax.axvline(
            ci_bounds[0], color="#2ca02c", linestyle="dashed", linewidth=2.2, label="Lower CI"
        )
        ax.axvline(
            ci_bounds[1], color="#ff7f0e", linestyle="dashed", linewidth=2.2, label="Upper CI"
        )
...
def plot_mahalanobis_outliers_3d(
    df,
    x_col="X1",
    y_col="X2",
    z_col="X3",
    ...
    fig.update_layout(
        ...
        scene=dict(xaxis=dict(title=x_col), yaxis=dict(title=y_col), zaxis=dict(title=z_col)),
    )

    return fig


def plot_mahalanobis_distance_distribution(
    df, distance_col="mahalanobis", threshold=None, title="Mahalanobis Distance Distribution"
):
    """
...
def plot_3d_loss_surface(X, Y, Z, optimum=None, title="Loss Surface 3D"):
    ...
    if optimum:
        ax.scatter(
            optimum[0], optimum[1], optimum[2], color="red", s=80, marker="o", label="Optimum"
        )
        ax.legend()
...
def plot_singular_values(singular_values, title="Singular Values"):
    ...
    sns.barplot(
        x=list(range(1, len(singular_values) + 1)), y=singular_values, palette="crest", ax=ax
    )
    ax.set_title(title, fontsize=12, fontweight="bold")
...
def plot_least_squares_residuals(x_values, residuals, title="Residuals of Least Squares Fit"):
    """
...
def plot_interpolation_comparison(
    x, y, x_new, y_lin, y_cub, y_spl, title="Interpolation Comparison", figsize=(10, 6)
):
    """
...
def plot_curve_fits_with_bands(
    x,
    y,
    x_new,
    y_exp_pred,
    ...
    y_exp_upper,
    y_gauss,
    title="Curve Fitting with Confidence Bands",
    figsize=(10, 6),
):
    """
    ...
    ax.fill_between(
        x_new, y_exp_lower, y_exp_upper, color="orange", alpha=0.2, label="Exp ±2σ Band"
    )
...
def plot_multivariate_griddata(
    grid_x,
    grid_y,
    grid_z,
...
def plot_rbf_interpolation(
    grid_x, grid_y, z_rbf, title="2D RBF Interpolation", cmap="plasma", figsize=(8, 6)
):
    """
...
def plot_gaussian_fit_with_band(
    x,
    y,
    x_new,
    y_pred,
...
def plot_polynomial_fit(
    x, y, x_new, y_poly, degree=2, title="Polynomial Curve Fit", figsize=(10, 6)
):
    """
...
def plot_all_fits_comparison(
    ...
    y_poly,
    figsize=(10, 6),
    title="All Fits Overlay Comparison",
):
    """
...
def plot_weighted_vs_unweighted_fit(
    x,
    y,
    x_new,
    y_unweighted,
    y_weighted,
    figsize=(10, 6),
    title="Weighted vs Unweighted Exponential Fit",
):
    """
...
def plot_multivariate_error_heatmap(
    grid_x, grid_y, error_grid, figsize=(8, 6), title="Interpolation Error Heatmap"
):
    """
...
def plot_confidence_interval(
    mean, ci, pop_mean=None, figsize=(6, 4), title="Confidence Interval for Sample Mean"
):
    """
    ...
    fig, ax = plt.subplots(figsize=figsize)
    ax.errorbar(
        1, mean, yerr=[[mean - ci[0]], [ci[1] - mean]], fmt="o", capsize=5, label="95% t-CI"
    )
...
def plot_singular_values_safe(singular_values, title="Singular Values (Safe Plot)"):
    ...
    if len(singular_values) > 0:
        ...
        ax.text(
            0.5, 0.5, "No valid singular values", ha="center", va="center", fontsize=12, color="red"
        )
        ax.set_axis_off()
