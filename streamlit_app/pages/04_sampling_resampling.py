...
if cat_counts.empty or poisson_df.empty:
    st.error(
        "Required datasets not found. Please ensure both categorical_counts.csv and poisson_data.csv are available."
    )
    st.stop()
    ...
    df_strat = pd.DataFrame(
        {
            "category": np.random.choice(
                cat_counts.index, p=cat_counts.values.flatten() / cat_counts.sum(), size=1000
            ),
...
if sampling_type != "Bootstrap (Poisson)" and result is not None:
    ...
    fig_freq = go.Figure()
    fig_freq.add_trace(
        go.Bar(x=freq_table.index.astype(str), y=freq_table.values, marker_color="steelblue")
    )
